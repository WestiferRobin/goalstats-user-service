...
def request_validation(error: ValidationError) -> Response:
    ...
    # flask-openapi3 forwards the model's error unchanged, without a body/path
    # prefix on locations. The model context distinguishes forbidden body keys.
    if error.title in {ItemPathSchema.__name__, ActionPathSchema.__name__}:

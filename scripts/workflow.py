...
IMAGE = "goalstats-user-py:tooling"
...
class Stack:
    def __init__(self, env, *, project=None, settings=None):
        ...
        self.env = env
        self.project = project or f"goalstats-user-py-{env}"
        self.disposable = project is not None
        if self.disposable and not re.fullmatch(
            r"goalstats-user-py-(test|cert)-[a-f0-9]+", project
        ):
...
def fresh(env):
    ...
    return Stack(
        env,
        project="goalstats-user-py-cert-" + secrets.token_hex(6),
        settings={
...
def tool_command(*command):
    name = "goalstats-user-py-tool-" + secrets.token_hex(6)
    try:
...
def tests(mode, *, fault=None, project=None):
    ...
    stack = Stack(
        "test",
        project=project or "goalstats-user-py-test-" + secrets.token_hex(6),
